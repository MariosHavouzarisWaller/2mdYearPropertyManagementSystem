...
public class RealEstateTest {
	...
	public void inOrderTest() {	
		...
		String expectedOutput = "AllanvaleRoad, Barton, Farlane, HendersonSt, HermitageRoad, KingsPark, McMarne, StNinians, TheThistlesCentre, ";
		String actualOutput = manager.inWalk();
		assertEquals(expectedOutput, actualOutput);		// [13] Checking to make sure the outputs equal each other
		System.out.println("\n--In Order Walk--\n" + manager.inWalk());
	}
	...
	public void binarySearchTest() {	
		...
		// Now conduct an assertion test...
		Property expectedOutput = new House("StNinians", 120, 4, 2);
		expectedOutput.setPrice(80000);
		Property actualOutput = manager.binarySearch(120);
		assertEquals(expectedOutput, actualOutput);		// [13] Checking to make sure the outputs equal each other
		System.out.println("\n--Binary Search (found)--\n" + manager.binarySearch(120));
		Property expectedOutput2 = manager.binarySearch(3122);
		assertNull(expectedOutput2);					// [13] Checking to make sure the wrong output is returned as Null
